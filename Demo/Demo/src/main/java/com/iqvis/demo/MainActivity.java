...
public class MainActivity extends AppCompatActivity implements ScanCallBack,ConnectionCallBack {
    ...
    IqvisBLE iqvisBLE;
    private Button rescan, connect, disconnect;
    private ListView device_list_view, service_list_view, characterstics_list_view;
    private TextView list_text, deviceName, deviceMac, deviceServices;
    ...
    private void initViews() {
        ...
        characterstics_list_view = (ListView) findViewById(R.id.charactersitics_list_view);
        list_text = (TextView) findViewById(R.id.textView_devices);

        deviceName = (TextView) findViewById(R.id.textView_ble_name);
        deviceMac = (TextView) findViewById(R.id.textView_ble_mac);
        deviceServices = (TextView) findViewById(R.id.textView_ble_services);
        setListeners();
    }
    ...
    private void setListeners() {
        ...
        service_list_view.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                characterstics_list_view.setVisibility(View.VISIBLE);
                ArrayAdapter<String> adapter = new ArrayAdapter<String>(MainActivity.this, android.R.layout.simple_list_item_1, charact);
                characterstics_list_view.setAdapter(adapter);
            }
        });
        rescan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                service_list_view.setVisibility(View.GONE);
                characterstics_list_view.setVisibility(View.GONE);
            }
        });
    }
...
//        super.onBackPressed();
        if (View.VISIBLE == device_list_view.getVisibility()) {
            ...
            service_list_view.setVisibility(View.GONE);
            device_list_view.setVisibility(View.VISIBLE);
        } else if (View.VISIBLE == characterstics_list_view.getVisibility()) {
            characterstics_list_view.setVisibility(View.GONE);
    ...
    public void onScanReturn(final List<BluetoothDevice> devices) {
        ...
        if (devices.size() == 0) {
            list_text.setText("No Device Found");
        } else {
            list_text.setText("Devices");
            device_list_view.setVisibility(View.VISIBLE);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ArrayList<String> BLEDevices = new ArrayList<String>(devices.size());
                    for (int i = 0; i < devices.size(); i++) {
                        BLEDevices.add(devices.get(i).getName().toString());
                    }
...
//                    ArrayAdapter<String> adapter = new ArrayAdapter<String>(MainActivity.this, android.R.layout.simple_list_item_1, BLEDevices);
                    ...
                    Toast.makeText(MainActivity.this, devices.get(0).getAddress().toString(), Toast.LENGTH_SHORT).show();
                }
            });
        }
        try {
...
//        list_text.setText("Connected");
    ...
    public void OnServicesDiscovered(List<BluetoothGattService> services) {
        ...
        serv = new String[services.size()];
        for (int i = 0; i < services.size(); i++) {
            serv[i] = services.get(i).toString();
        }
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(MainActivity.this, android.R.layout.simple_list_item_1, serv);
